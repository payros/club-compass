...
export async function GET() {
  const clubYearList = await clubYearsService.list()
...
export async function POST(request) {
  ...
  try {
    const clubYearRecord = await clubYearsService.create(newClubYearData)
    ...
    console.error(err)
    return NextResponse.json({ error: 'Failed to create club year.' }, { status: 500 })
  }
}
