...
async function getById(eventId) {
  try {
    const result = await sql`SELECT ev.id, ev.title, ev.event_date, ev.award_ceremony,
                ...
                coalesce(json_agg(DISTINCT jsonb_build_object('id', a.id, 'name', a.name, 'classId', ea.class_id, 'class', cl2.class, 'type', a.type, 'patchImageUrl', a.patch_image_url)) FILTER (WHERE a.id IS NOT NULL), '[]'::json) AS awards
                FROM adv_db.events as ev
                JOIN adv_db.club_years as cy ON ev.club_year_id = cy.id
...
async function update(clubYearLabel, eventId, updatedEventData) {
  try {
    const result = await sql.begin(async (sql) => {
      ...
      // Extract children and awards arrays if present
      const { children, awards, ...eventData } = updatedEventData
      ...
      Object.entries(eventData)
        ...
        // Also exclude awards — handled separately below.
        .filter(([key]) => key !== 'award_ceremony' && key !== 'awards')
        .forEach(([key, value]) => {
      ...
      // Handle awards change if provided
      if (awards !== undefined) {
        // Get current events_awards rows
        const currentEventAwards = await sql`
          SELECT award_id, class_id
          FROM adv_db.events_awards
          WHERE event_id = ${eventIdInt}`

        // Find removed (award_id, class_id) pairs
        const removedPairs = currentEventAwards.filter(
          (cur) =>
            !awards.some(
              (nw) =>
                parseInt(nw.award_id, 10) === cur.awardId &&
                (nw.class_id === null || nw.class_id === undefined
                  ? cur.classId === null
                  : parseInt(nw.class_id, 10) === cur.classId)
            )
        )

        // Find added (award_id, class_id) pairs
        const addedPairs = awards.filter(
          (nw) =>
            !currentEventAwards.some(
              (cur) =>
                parseInt(nw.award_id, 10) === cur.awardId &&
                (nw.class_id === null || nw.class_id === undefined
                  ? cur.classId === null
        ...
        // Handle removed pairs: clean up awards_children, then delete from events_awards
        for (const { awardId, classId } of removedPairs) {
          if (updatedEvent.awardCeremony) {
            if (classId === null) {
              await sql`
                UPDATE adv_db.awards_children
                SET awarded_on = NULL, award_ceremony_id = NULL
                WHERE award_id = ${awardId} AND award_ceremony_id = ${eventIdInt}`
            } else {
              await sql`
                UPDATE adv_db.awards_children
                SET awarded_on = NULL, award_ceremony_id = NULL
                WHERE award_id = ${awardId} AND award_ceremony_id = ${eventIdInt}
                  AND child_id IN (
                    SELECT child_id FROM adv_db.classes_children WHERE class_id = ${classId}
                  )`
            }
          } else {
            if (classId === null) {
              await sql`
                DELETE FROM adv_db.awards_children
                WHERE award_id = ${awardId} AND event_id = ${eventIdInt}`
            } else {
              await sql`
                DELETE FROM adv_db.awards_children
                WHERE award_id = ${awardId} AND event_id = ${eventIdInt}
          ...
          // Delete from events_awards
          if (classId === null) {
            await sql`
              DELETE FROM adv_db.events_awards
              WHERE event_id = ${eventIdInt} AND award_id = ${awardId} AND class_id IS NULL`
          } else {
            await sql`
              DELETE FROM adv_db.events_awards
              WHERE event_id = ${eventIdInt} AND award_id = ${awardId} AND class_id = ${classId}`
          }
        }

        // Handle added pairs: insert into events_awards, then link to attending children
        if (addedPairs.length > 0) {
          const newEventAwardValues = addedPairs.map((a) => [
            eventIdInt,
            parseInt(a.award_id, 10),
            a.class_id ? parseInt(a.class_id, 10) : null,
          ])
          await sql`
            INSERT INTO adv_db.events_awards (event_id, award_id, class_id)
            VALUES ${sql(newEventAwardValues)}`

          // Get attending children with their classes for this club year
          const attendeesWithClasses = await sql`
            SELECT ec.child_id, cc.class_id
            FROM adv_db.events_children ec
            JOIN adv_db.classes_children cc ON ec.child_id = cc.child_id
            JOIN adv_db.club_years cy ON cc.club_year_id = cy.id
            WHERE ec.event_id = ${eventIdInt} AND cy.label = ${clubYearLabel}`

          // Build awards_children rows for each added (award_id, class_id) pair
          const awardsToAdd = []
          for (const { award_id, class_id } of addedPairs) {
            const awardIdInt = parseInt(award_id, 10)
            const classIdInt = class_id ? parseInt(class_id, 10) : null
            const matching =
              classIdInt === null ? attendeesWithClasses : attendeesWithClasses.filter((a) => a.classId === classIdInt)
            for (const { childId } of matching) {
              awardsToAdd.push([childId, awardIdInt, eventIdInt])
            }
          }

          // Deduplicate by (child_id, award_id, event_id)
          const uniqueAwardsToAdd = awardsToAdd.filter(
            (item, i, self) => i === self.findIndex((t) => t[0] === item[0] && t[1] === item[1] && t[2] === item[2])
          )

          if (uniqueAwardsToAdd.length > 0) {
            if (updatedEvent.awardCeremony) {
              const awardUpdateFields = []
              const awardUpdateValues = []
              let paramIdx = 1
              uniqueAwardsToAdd.forEach(([childId, awardId]) => {
                awardUpdateFields.push(`(child_id = $${paramIdx++} AND award_id = $${paramIdx++})`)
                awardUpdateValues.push(childId, awardId)
              })
              const updateQuery = `
                UPDATE adv_db.awards_children
