...
export async function POST(request, { params }) {
  ...
  try {
    ...
    key = await storage.put(`awards/${id}/patch.webp`, webpBuffer)
  } catch (err) {
    console.error('[photo upload] storage error:', err)
    return NextResponse.json({ error: 'Failed to store image.' }, { status: 500 })
  }

  try {
    await awardsService.updatePatchImageUrl(id, key)
  } catch (err) {
    console.error('[photo upload] db error:', err)
    return NextResponse.json({ error: 'Image stored but DB update failed.' }, { status: 500 })
  }

  const patchImageUrl = await resolveImageUrl(key)
  return NextResponse.json({ patchImageUrl })
}
...
export async function DELETE(request, { params }) {
  ...
  if (award.patchImageUrl) {
    const key = `awards/${id}/patch.webp`
    try {
      const storage = await getStorageAdapter()
      await storage.remove(key)
    } catch (err) {
      console.error('[photo delete] storage error:', err)
      // Non-fatal: still clear the DB field
    }
  }

  try {
    await awardsService.updatePatchImageUrl(id, null)
  } catch (err) {
    console.error('[photo delete] db error:', err)
